...
- **FR43:** **Phase-1 action lockdown (scope = who, not how much).** During Phase 1, the entire AI write/action surface (all write tools — existing and new — plus the agentic engine, CRUD, and self-learning) is authorized for **Owner + Principal only** — but those two profiles act across the **whole school database** they are authorized over (full breadth, all domains). Reach follows existing authority/tenancy unchanged: Owner = school-wide incl. cross-branch; Principal = authorized scope via `scoped_query`. The lockdown restricts the acting roles, not the data breadth. Other staff roles do not receive AI write/action access during the pilot, even where the REST route permits them; this is enforced as a single, reversible authorization policy widened in Phase 2 (Epic H) for **staff roles only**. **Students are exempt and unchanged** — their existing read-only, content-filtered experience is preserved as-is and is permanently out of scope for the write/action/self-learning expansion.
