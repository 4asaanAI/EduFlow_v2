...
- **Phase-1 authorization LOCKDOWN (user, 2026-06-07, Story F.11):** the ENTIRE AI write/action surface — every new CRUD tool AND the existing write tools (Epics A–C) — is gated to **Owner + Principal ONLY** during Phase 1 (`role=owner` OR `role=admin`+`sub_category=principal`), enforced once in `_is_tool_authorized` as a single reversible policy, even where the REST route permits broader roles (fee config → accountant, staff → admin, teacher → attendance). Other staff roles are deferred to Phase 2 (Epic H), which widens the policy. **Students are exempt: their existing read-only/content-filtered tools are unchanged and are never granted write/action access (now or in Phase 2).** Org-level config (branches/settings) stays owner-only even in Phase 2.
