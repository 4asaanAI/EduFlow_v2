...
>
> **📚 Phase-1 DATA scope = the WHOLE school database (user, 2026-06-07):** the lockdown limits *who* acts (Owner + Principal), **not how much data they reach**. Within Phase 1 these two profiles can read and act across the entire school database they are authorized over — students, attendance, fees/discounts, academics, classes/sections/houses, staff, announcements, incidents, branches, settings, etc. **Reach follows existing authority/tenancy (unchanged):** Owner = school-wide incl. **cross-branch** (`branch_id=None`); Principal = their authorized scope via `scoped_query(branch_id=…)`. So "whole school database" is the full breadth each is *authorized* over — not a blanket cross-branch grant to a branch-scoped principal. DPDP minor-data controls (FR19–23) still apply to every access.
