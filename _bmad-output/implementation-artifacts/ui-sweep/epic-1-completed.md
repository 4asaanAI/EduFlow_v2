...
quietly granting the person nothing. And every member of staff can now correct
their own name, phone number and email themselves, without being able to change
...
Staff had **no** self-service endpoint at all before this story (students did).
New: `GET /api/staff/me` and `PATCH /api/staff/me`.

| AC | Met | How |
|---|---|---|
| Name, phone, email editable; role/school/sub-category/token allowance read-only | ✅ | `SELF_SERVICE_FIELDS` allow-list, server-side; the dialog renders the rest behind a lock icon |
| A change to own role / sub_category / schoolId / token allowance is refused | ✅ | Parametrised over six privileged fields |
| A mixed request is refused **whole**, not partly applied | ✅ | `{name, role}` saves neither — partial acceptance maps the boundary for the caller |
| Someone with no staff record gets a clear 404, no record conjured | ✅ | tested |
| Audited | ✅ | `action="self_update"` |
| Visible immediately without a reload | ✅ | `applyProfileUpdate()` takes the new values from the server's response, not the token, which stays stale until the next sign-in |
| Survives sign-out and sign-in | ✅ | The login record's `user_info` is updated too |
| `data-testid`, CSS variables, visible focus at ≥3:1 | ✅ | Every control; reuses the existing `.focus-ring` utility |
| The dialog no longer prints a hard-coded wrong city | ✅ | D-11 |

**Scope decision (party mode):** self-service only. Changing another person's
authority stays on the staff screen, so there is exactly one place in the
platform where a person's permissions can move.

---

## Tests

New file `tests/backend/api/test_ui_sweep_epic1_access.py` — 44 tests.
Rewritten (not deleted) to the stronger contract: 3 tests in
`test_staff_routes.py` and `test_epic_j_crud_guardrails.py` (D-14).

**Full suite: 1682 passed, 2 failed, 14 deselected.** The 2 are the pinned,
...
failed; +46 passing tests, all accounted for.
