...
import { X, Mail, Phone, Shield, Zap } from 'lucide-react';
...
export default function ProfileModal({ onClose }) {
  ...
  const text   = isDark ? '#f5f5f5' : '#171717';
  const muted  = isDark ? '#888'    : '#525252';
  const sec    = isDark ? '#a0a0a0' : '#525252';
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300 }}>
      ...
      >
        {isStudent ? (
          ...
          /* Staff / teacher / owner: read-only view */
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0, marginBottom: 20 }}>
              {[
                ...
                <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: `1px solid ${border}` }}>
                  ...
                  <span style={{ fontSize: 13, color: sec, fontWeight: 500 }}>{item.value}</span>
                </div>
              ))}
            </div>
