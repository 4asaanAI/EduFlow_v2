...
# The onboarding steps a school must complete before it auto-activates.
...
# ─── School onboarding (Story 7-44) ───────────────────────────────────────────
#
# The `schools` registry is a PLATFORM-level, cross-tenant collection (keyed on
# `school_id`, NOT the per-tenant `schoolId`). These endpoints therefore use
# get_raw_db() — the school being provisioned/inspected is never the caller's own
# tenant, so the scoped DB would wrongly inject the operator's schoolId filter.

def _generate_temp_password(length: int = 12) -> str:
    """A 12-char temporary password from a URL-safe alphabet."""
    alphabet = string.ascii_letters + string.digits
    return "".join(secrets.choice(alphabet) for _ in range(length))


async def _send_operator_slack(school_name: str, school_id: str) -> None:
    """Best-effort Slack ping when a school finishes onboarding (fail-open)."""
    webhook = os.environ.get("OPERATOR_SLACK_WEBHOOK_URL")
    if not webhook:
        return
    try:
...
async def create_school(request: Request, user: dict = Depends(require_owner)):
    ...
    # Welcome email is best-effort — provisioning must not fail if SMTP is down.
    try:
...
async def deactivate_school(school_id: str, user: dict = Depends(require_owner)):
    """Deactivate a school (owner-only)."""
    raw_db = get_raw_db()
    school = await raw_db.schools.find_one({"school_id": school_id})
    if not school:
        raise HTTPException(status_code=404, detail=f"School '{school_id}' not found")
    ...
    # Revoke every refresh token for this school's users so a deactivation takes
    # effect immediately (no lingering sessions on access-token expiry).
