...
async def connect_db():
    ...
    try:
        ...
        create_indexes = os.environ.get("CREATE_INDEXES_ON_STARTUP", "").lower()
        if create_indexes == "true" or (not create_indexes and environment in {"development", "test", "testing"}):
            await _create_indexes()
        else:
            logger.info(
                "Skipping index creation on startup; run registered migrations explicitly"
            )
    except Exception:
        logger.exception("MongoDB connection or index initialization failed")
        raise
...
async def _create_indexes():
    ...
    # Enterprise school lifecycle indexes (additive; no data migration is run at startup).
    ...
    await db.student_leave_days.create_index(
        [("student_id", 1), ("date", 1)], unique=True
    )
    ...
    await db.legal_entities.create_index(
        [("schoolId", 1), ("branch_id", 1), ("is_default", 1)], unique=True,
